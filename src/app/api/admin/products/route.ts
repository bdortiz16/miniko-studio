...
export async function POST(request: Request) {
  ...
  if (body.id) {
    ...
    list[idx] = {
      ...
      images: Array.isArray(body.images) ? body.images.filter(Boolean).slice(0, 6) : list[idx].images,
  ...
  const product: Product = {
    ...
    priceCop,
    image: body.image || "",
    images: Array.isArray(body.images) ? body.images.filter(Boolean).slice(0, 6) : undefined,
    category: (body.category || "").trim() || undefined,
    active: body.active !== false,
